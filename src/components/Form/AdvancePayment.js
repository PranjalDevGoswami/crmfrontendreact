...
const AdvancePayment = ({abrData,setAbrData}) => {
  const { formData, setAdvancePayment,managerList, } = useContext(FormDataContext);
  ...
  useEffect(() => {
    ...
    const selectedManager = managerList?.filter((manager)=>manager.user_role.id == formData?.project_manager)
    const selectedManagerName = selectedManager?.map((user)=>user?.user_role?.name)
  ...
  const validateForm = () => {
    if (!abrData?.total_project_cost) return "Total Cost of Project is required.";
    if (!abrData?.advance_invoice_percentage) return "Advance Invoice Percentage is required.";
    if (!abrData?.advance_invoice_amount) return "Advance Invoice Amount is required.";
    if (!abrData?.contact_person_name) return "Name Required.";
    if (!abrData?.contact_person_email) return "Email Required.";
    if (!abrData?.total_project_cost) return "Total Cost of Project (Amount) Required.";
    return "";
  };
