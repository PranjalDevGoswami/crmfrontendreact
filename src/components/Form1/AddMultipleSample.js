  ...
  return (
    <div className="p-4 bg-white">
          ...
          <div key={rowIndex} className="flex items-center mb-2">
                ...
                .map((item, index) => (
                  <div key={index} className="w-1/3 pr-2">
                    <LableAndInput
                      ...
                      InputType={item.type}
                      inputClassName={"p-1 px-4 border bg-white w-full rounded-none"}
            ...
            {Math.ceil(inputData.length / 3) > 1 && (
              <Button
                ...
                className={
                  "bg-red-400 text-white p-1 border rounded-none text-xs mt-[30px] w-1/12"
                }
              />
            )}
            {rowIndex === Math.ceil(inputData.length / 3) - 1 && (
              <div className="w-2/12">
                <Button
                  ...
                  className={
                    "bg-green-400 text-white p-1 border rounded-none w-2/3 text-xs mt-[30px]"
                  }
                />
              </div>
            )}
      ...
      <Button
        ...
        className={"bg-green-300 p-2 text-white mt-4 cursor-pointer rounded-none"}
      />
      <button
        onClick={handleCross}
        className="absolute top-2 right-2 p-1 bg-red-400 text-white text-xs rounded-none cursor-pointer"
      >
        X
      </button>
    </div>
